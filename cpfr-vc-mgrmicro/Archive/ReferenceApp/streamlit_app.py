...
def test_permissions():
    ...
    st.subheader("🔧 Permission & Connection Test")
    
    try:
        session = st.session_state.db_manager.get_session()
        
        # Test basic connection
        st.write("**1. Basic Connection Test**")
        try:
            test_result = session.sql("SELECT 1 as test").collect()
            st.write("✅ Basic connection: OK")
        except Exception as e:
            st.write(f"❌ Basic connection: FAILED - {e}")
            return
        
        # Test user context
        st.write("**2. User Context Test**")
        try:
            user_info = session.sql("SELECT CURRENT_USER(), CURRENT_ROLE()").collect()
            current_user = user_info[0][0] if user_info else "Unknown"
            current_role = user_info[0][1] if user_info else "Unknown"
            st.write(f"✅ User: {current_user}, Role: {current_role}")
        except Exception as e:
            st.write(f"❌ User context: FAILED - {e}")
        
        # Test SELECT permission
        st.write("**3. SELECT Permission Test**")
        try:
            test_select = session.sql("SELECT COUNT(*) FROM EDLDB.SC_SANDBOX.VC_CPFR_VENDOR_EMAIL WHERE 1=0").collect()
            st.write("✅ SELECT permission: OK")
        except Exception as e:
            st.write(f"❌ SELECT permission: FAILED - {e}")
        
        # Test UPDATE permission
        st.write("**4. UPDATE Permission Test**")
        try:
            test_update = session.sql("UPDATE EDLDB.SC_SANDBOX.VC_CPFR_VENDOR_EMAIL SET \"OVERRIDE_EMAIL\" = \"OVERRIDE_EMAIL\" WHERE 1=0").collect()
            st.write("✅ UPDATE permission: OK")
        except Exception as e:
            st.write(f"❌ UPDATE permission: FAILED - {e}")
        
        # Test INSERT permission
        st.write("**5. INSERT Permission Test**")
        try:
            # Test INSERT permission by trying to insert into a non-existent record
            test_insert = session.sql("INSERT INTO EDLDB.SC_SANDBOX.VC_CPFR_VENDOR_EMAIL (\"Vendor Number\", \"FILE\") VALUES ('TEST123', 'TEST')").collect()
            st.write("✅ INSERT permission: OK")
            
            # Clean up the test record
            try:
                session.sql("DELETE FROM EDLDB.SC_SANDBOX.VC_CPFR_VENDOR_EMAIL WHERE \"Vendor Number\" = 'TEST123' AND \"FILE\" = 'TEST'").collect()
                st.write("✅ Test record cleaned up")
            except:
                st.write("⚠️ Could not clean up test record (this is OK)")
                
        except Exception as e:
            st.write(f"❌ INSERT permission: FAILED - {e}")
        
        # Test specific vendor access
        st.write("**6. Specific Vendor Access Test**")
        try:
            vendor_check = session.sql("SELECT COUNT(*) FROM EDLDB.SC_SANDBOX.VC_CPFR_VENDOR_EMAIL WHERE \"Vendor Number\" = '4468' AND \"FILE\" = 'Tier2'").collect()
            count = vendor_check[0][0] if vendor_check else 0
            st.write(f"✅ Vendor 4468/Tier2 exists: {count > 0} (count: {count})")
        except Exception as e:
            st.write(f"❌ Vendor access: FAILED - {e}")
        
        # Test actual UPDATE on test data
        st.write("**7. Actual UPDATE Test**")
        try:
            # First, get current value
            current_value = session.sql("SELECT \"OVERRIDE_EMAIL\" FROM EDLDB.SC_SANDBOX.VC_CPFR_VENDOR_EMAIL WHERE \"Vendor Number\" = '4468' AND \"FILE\" = 'Tier2'").collect()
            st.write(f"Current OVERRIDE_EMAIL: {current_value[0][0] if current_value else 'None'}")
            
            # Try to update
            update_result = session.sql("UPDATE EDLDB.SC_SANDBOX.VC_CPFR_VENDOR_EMAIL SET \"OVERRIDE_EMAIL\" = 'test@example.com' WHERE \"Vendor Number\" = '4468' AND \"FILE\" = 'Tier2'").collect()
            st.write(f"✅ UPDATE test: SUCCESS (rows affected: {len(update_result) if update_result else 0})")
            
            # Check if it actually changed
            new_value = session.sql("SELECT \"OVERRIDE_EMAIL\" FROM EDLDB.SC_SANDBOX.VC_CPFR_VENDOR_EMAIL WHERE \"Vendor Number\" = '4468' AND \"FILE\" = 'Tier2'").collect()
            st.write(f"New OVERRIDE_EMAIL: {new_value[0][0] if new_value else 'None'}")
            
            # Clean up the test data (set back to NULL)
            try:
                session.sql("UPDATE EDLDB.SC_SANDBOX.VC_CPFR_VENDOR_EMAIL SET \"OVERRIDE_EMAIL\" = NULL WHERE \"Vendor Number\" = '4468' AND \"FILE\" = 'Tier2'").collect()
                st.write("✅ Test data cleaned up (set back to NULL)")
            except:
                st.write("⚠️ Could not clean up test data (this is OK)")
            
        except Exception as e:
            st.write(f"❌ UPDATE test: FAILED - {e}")
...
def reset_application_state():
    """
    Force a complete reset of the application by setting a reset flag.
    The flag is checked at the very top of the script (before any initialization)
    and triggers a complete state clear and rerun.
    """
    # Set the reset flag - this will be checked at the top of the script
    st.session_state._force_reset = True
    # The rerun will happen automatically when the flag is detected
...
def main():
    ...
    st.title("📧 CPFR Vendor Contact Manager")
    st.markdown("Manage vendor contact information in the VC_CPFR_VENDOR_EMAIL table")
    
    # Add a test button for debugging
    if st.button("🔧 Test Permissions & Connection"):
        test_permissions()
    
    # Simplified user context display
    try:
        session = st.session_state.db_manager.get_session()
        ...
        try:
            ...
            st.sidebar.info("**Connected to Snowflake** (User context unavailable)")
...
def show_search_screen():
    ...
    search_type = st.selectbox(
        ...
        help="Select the field to search against",
    ...
    if search_type == "Vendor Number":
        ...
        if (hasattr(st.session_state, 'search_performed') and 
            st.session_state.search_performed and
            ...
            if search_value:
                ...
                st.caption("*Enter a vendor number and click 'Search' to create a new entry*")
...
def display_dual_entry_results(search_result: VendorSearchResult):
    ...
    st.subheader("📋 Dual Entry Results")
...
def show_edit_screen():
    ...
    if file_value in ['Tier2', '6Months']:
        try:
            ...
            if tier2_entry and sixmonths_entry:
                ...
                mismatches = []
                for field in ['Vendor Name', 'PURCHASER', 'Shipment Method Code', 'Vendor Contacts', 
                             'Parent Vendor', 'CB Rollout Phase', 'Soft Chargeback Effective Date',
    ...
    with st.form("vendor_edit_form"):
        ...
        # Determine available FILE options and which to disable
        ...
        # Create FILE selectbox with conditional options
        if disable_tier2:
            # Filter out Tier2 option when editing 6Months tied to Tier2
            filtered_options = [opt for opt in file_options if opt != 'Tier2']
            file_index = filtered_options.index(current_file) if current_file in filtered_options else 0
            updated_file = st.selectbox(
                "FILE",
                filtered_options,
                index=file_index,
                help="⚠️ Cannot change 6Months entry to Tier2 when a Tier2 entry already exists for this vendor. Changing to Tier1 or 3Months will merge both entries into a single entry.",
                key="edit_file_field"
            )
        else:
            ...
            updated_file = st.selectbox(
                ...
                file_options,
                index=file_index,
                help="Tier2 will automatically create both Tier2 and 6Months entries. Changing from Tier2 to another tier will merge both entries into a single entry.",
                key="edit_file_field"
            )
        ...
        for field in editable_fields:
            ...
            display_value = current_value if current_value is not None and str(current_value).strip() != '' else ''
    ...
    if 'pending_changes' in st.session_state and st.session_state.pending_changes:
        ...
        with col2:
            if st.button("❌ Cancel Changes"):
                if 'pending_changes' in st.session_state:
                    del st.session_state.pending_changes
                if 'original_vendor' in st.session_state:
                    del st.session_state.original_vendor
                st.rerun()
...
def confirm_vendor_changes():
    ...
    try:
        ...
        if file_changing and original_file != new_file:
            try:
                # Store last error in session state for debugging
                if 'last_db_error' in st.session_state:
                    del st.session_state.last_db_error
...
def show_receipt_screen():
    ...
    with col1:
        if st.button("✏️ Continue Editing", type="primary"):
            # Return to edit mode with updated vendor
            st.session_state.selected_vendor = updated_vendor
            st.session_state.current_mode = 'edit'
    ...
    with col2:
        if st.button("🔍 Back to Search", type="secondary"):
            # Clear everything and go to search
            st.session_state.current_mode = 'search'
            st.session_state.tier_change_receipt = None
            st.session_state.selected_vendor = None
            if 'search_results' in st.session_state:
    ...
    with col3:
        if st.button("📋 View Search Results", type="secondary"):
...
def show_new_entry_screen():
    ...
    with st.form("new_vendor_form"):
        ...
        file_value = st.selectbox(
            ...
            help="Tier2 will automatically create both Tier2 and 6Months entries"
        )
        
        # Show warning for Tier2
        if file_value == "Tier2":
            st.warning("⚠️ Selecting Tier2 will create both Tier2 and 6Months entries with identical data")
    ...
    if back_clicked:
        ...
        st.session_state.selected_vendor = None
        st.session_state.search_value = ''
        if 'new_vendor_data' in st.session_state:
            del st.session_state.new_vendor_data
        if 'new_vendor_file' in st.session_state:
            del st.session_state.new_vendor_file
        st.rerun()
