...
class VendorProcessor:
    ...
    def should_create_dual_entry(self, file_value: str) -> bool:
        """
        Check if FILE value requires dual entry creation
        
        Args:
            file_value: FILE field value
            
        Returns:
            True if dual entry should be created
        """
        return file_value == 'Tier2'
    ...
    def format_vendor_display(self, vendor: Dict[str, Any]) -> str:
        """
        Format vendor for display in search results
        
        Args:
            vendor: Vendor data dictionary
            
        Returns:
            Formatted string for display
        """
        vendor_number = vendor.get('Vendor Number', 'Unknown')
        vendor_name = vendor.get('Vendor Name', 'Unknown')
        file_value = vendor.get('FILE', 'Unknown')
        
        return f"{vendor_number} - {vendor_name} ({file_value})"
    ...
    def get_editable_fields(self) -> List[str]:
        ...
        """
        return [
            'Vendor Name', 'PURCHASER', 'Shipment Method Code', 'Vendor Contacts',
