...
**CRITICAL**: When building Streamlit apps for Snowflake, use patterns compatible with **Streamlit in Snowflake** (the native Snowflake Projects environment), NOT standalone Streamlit or the older Snowflake Native Apps variant.

**Key Differences**:
- **Streamlit in Snowflake**: Uses `st.connection()` or `get_active_session()` from `snowflake.snowpark.context`
- **Standalone Streamlit**: Uses `snowflake.connector` with explicit credentials
- **Snowflake Native Apps**: Uses different connection patterns (deprecated approach)

**Correct Pattern for Streamlit in Snowflake**:
```python
import streamlit as st
from snowflake.snowpark.context import get_active_session

# Option 1: Use st.connection() (recommended for Streamlit in Snowflake)
@st.cache_resource
def get_snowflake_connection():
    """Get Snowflake connection using Streamlit in Snowflake pattern"""
    return st.connection("snowflake")

# Option 2: Use get_active_session() (alternative pattern)
def get_session():
    """Get active Snowpark session (lazy initialization)"""
    try:
        from snowflake.snowpark.context import get_active_session
        session = get_active_session()
        if not session:
            raise ConnectionError("No active Snowflake session. Must run in Streamlit in Snowflake.")
        return session
    except ImportError:
        raise ImportError("Snowflake Snowpark not available. Must run in Streamlit in Snowflake.")

# CRITICAL: Initialize connections LAZILY, not in __init__ or module-level code
# BAD: session = get_active_session()  # At module level - can hang on load
# GOOD: session = get_session()  # Inside a function when needed
```

**Anti-Patterns to Avoid**:
1. **Eager Initialization**: Don't call `get_active_session()` or run test queries in `__init__` or at module level
   - BAD: `class DB: def __init__(self): self.session = get_active_session(); self.session.sql("SELECT 1").collect()`
   - GOOD: `class DB: def get_session(self): return get_active_session() if not self._session else self._session`
2. **Blocking Test Queries**: Don't run `.collect()` during initialization - this can hang the app
3. **Standalone Streamlit Patterns**: Don't use `snowflake.connector.connect()` with credentials in Streamlit in Snowflake
4. **State Machine Issues**: Use Streamlit's session state properly - avoid complex state machines that conflict with Streamlit in Snowflake's rerun behavior

**Why This Matters**:
- Streamlit in Snowflake manages connections automatically
- Eager initialization can cause apps to hang on load
- Connection patterns differ significantly from standalone Streamlit
- State management must work with Streamlit in Snowflake's execution model

**Common Connection Issues & Solutions**:

1. **"Unable to connect to the Snowflake backend" Error**:
   - **Most Common Cause**: Network/firewall blocking `*.snowflake.app` or `*.snowflake.com` domains
   - **Solution**: Ensure corporate firewalls allowlist these domains and WebSocket connections
   - **Browser Extensions**: Some extensions (especially security/content filters) can block Local Network Access (LNA)
   - **Chrome LNA**: Chrome 142+ requires LNA permission for Snowsight - check `chrome://flags#local-network-access-check`
   - **Office vs Home**: Different network configurations can cause the same code to work in one environment but not another

2. **Defensive Error Handling Pattern**:
   ```python
   # Always wrap DatabaseManager initialization in try/except
   if 'db_manager' not in st.session_state:
       try:
           st.session_state.db_manager = DatabaseManager()
       except Exception as e:
           logger.error(f"DatabaseManager initialization failed: {e}")
           st.session_state.db_manager = None  # Allow app to load, show error in UI
   
   # In main(), check and retry if needed
   if st.session_state.db_manager is None:
       try:
           st.session_state.db_manager = DatabaseManager()
       except Exception as init_error:
           st.error(f"Failed to connect to Snowflake: {init_error}")
           return  # Graceful failure with user-friendly message
   ```

3. **Testing Connection Issues**:
   - Create a minimal test app to isolate connection problems
   - Test from different networks (office, home, VPN) to identify firewall issues
   - Check browser console for WebSocket connection errors
   - Verify Snowflake warehouse is running and accessible
