...
class DatabaseManager:
    ...
    def _check_dual_entry_exists(self, vendor_number: str) -> Tuple[bool, bool]:
        """Helper to check if Tier2 and 6Months entries exist for a vendor"""
        ...
        return tier2_exists, sixmonths_exists
    ...
    def change_vendor_tier(self, vendor_number: str, original_file: str, new_file: str, updates: Dict[str, Any]) -> bool:
        ...
        if new_file == 'Tier2':
            tier2_exists, sixmonths_exists = self._check_dual_entry_exists(vendor_number)
    ...
    def _change_to_tier2_with_dual_entry(self, vendor_number: str, original_file: str, updates: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            # Get vendor_processor from streamlit session state
            import streamlit as st
            vendor_processor = st.session_state.get('vendor_processor')
            if not vendor_processor:
                logger.error("VendorProcessor not available in session state")
                return False
            tier2_data, sixmonths_data = vendor_processor.create_dual_entry_data(updated_vendor_data)
    ...
    def _check_and_fix_unsynced_dual(self, vendor_number: str) -> bool:
        ...
        try:
            ...
            for field in all_fields:
                if field == 'FILE':
                    continue
                # Skip deprecated columns - they're being removed and shouldn't be checked for mismatches
                if field in self.DEPRECATED_COLUMNS:
                    continue
                tier2_value = tier2_entry.get(field)
                sixmonths_value = sixmonths_entry.get(field)
                ...
                for field in email_fields:
                    if field in merged_data and merged_data[field]:
                        try:
                            ...
                            vendor_processor = st.session_state.get('vendor_processor')
                            if vendor_processor:
                                result = vendor_processor.email_validator.validate_email_list(str(merged_data[field]))
    ...
    def _insert_dual_entry_vendor(self, vendor_data: Dict[str, Any]) -> bool:
        ...
        if not vendor_processor:
            logger.error("VendorProcessor not available in session state")
            return False
        tier2_data, sixmonths_data = vendor_processor.create_dual_entry_data(vendor_data)
